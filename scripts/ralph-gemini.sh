...
if [ -f "$_SCRIPT_DIR/prd.json" ]; then
    RALPH_DIR="$_SCRIPT_DIR"
else
    RALPH_DIR="$(cd "$_SCRIPT_DIR/.." && pwd)"
fi
...
GEMINI_MODEL="${GEMINI_MODEL_ARG:-${GEMINI_MODEL:-gemini-2.5-pro}}"
...
: "${PRD_BACKUP:=$RALPH_DIR/prd.backup}"
: "${PROMPT_FILE:=$RALPH_DIR/prompt.md}"
: "${PROGRESS_FILE:=$RALPH_DIR/progress.txt}"

CONFIG_LOADED=false
if [ -n "$CONFIG_FILE" ]; then
    [[ "$CONFIG_FILE" != /* ]] && CONFIG_FILE="$RALPH_DIR/$CONFIG_FILE"
    if [ ! -f "$RALPH_DIR/ralph-config.sh" ]; then
        echo "ralph-config.sh not found in $RALPH_DIR" >&2
        exit 1
    fi
...
watch_mode() {
    echo -e "${CYAN}===============================================${NC}"
    echo -e "  Ralph (Gemini) Watch Mode"
    echo -e "${CYAN}===============================================${NC}"
    echo ""
...
status_mode() {
    ...
    echo -e "   ${GREEN}* $GEMINI_MODEL (active)${NC}"

    if [ -f "$PRD_FILE" ]; then
        local prd_git_dir
        prd_git_dir=$(jq -r '.workingDirectory // empty' "$PRD_FILE" 2>/dev/null)
        if [ -n "$prd_git_dir" ] && [ -d "$prd_git_dir/.git" ]; then
            echo ""
            local commits=$(git -C "$prd_git_dir" log --oneline --since="today" 2>/dev/null | wc -l | tr -d ' ')
...
echo -e "${CYAN}===============================================${NC}"
echo -e "  Ralph (Gemini) - Autonomous AI Agent"
echo -e "${CYAN}===============================================${NC}"
echo ""
...
echo -e "${GRAY}   Gemini output: $RALPH_DIR/gemini_output.log${NC}"
echo -e "${GRAY}   Gemini last msg: $RALPH_DIR/gemini_last_message.txt${NC}"
...
log_activity "=== Ralph (Gemini) session started ==="
...
if ! command -v gemini &> /dev/null; then
    log_error "Gemini CLI not found. Install the Google Gemini CLI and ensure gemini is on PATH."
    exit 1
fi

GEMINI_VERSION=$(gemini --version 2>/dev/null || echo "unknown")
log_success "Using Gemini CLI ($GEMINI_VERSION) with model $GEMINI_MODEL"

if [ -z "$GEMINI_API_KEY" ] && [ -z "$GOOGLE_API_KEY" ] && [ -z "$GOOGLE_APPLICATION_CREDENTIALS" ]; then
    log_warning "No Gemini auth found. Set GEMINI_API_KEY, GOOGLE_API_KEY, or GOOGLE_APPLICATION_CREDENTIALS."
    log_info "Run gemini auth login or set the appropriate environment variable."
fi
...
parse_error_type() {
    ...
    if echo "$error_output" | grep -qiE "ModelNotFoundError|model.?not.?found|does.?not.?exist|invalid.?model|NOT_FOUND.*model"; then
        echo "MODEL_ERROR"
    elif echo "$error_output" | grep -qiE "Cannot read properties of undefined|unexpected critical error|TypeError:"; then
        echo "SERVER_ERROR"
    elif echo "$error_output" | grep -qiE "Terminated|SIGTERM|SIGKILL|signal|killed"; then
        echo "TERMINATED"
    elif echo "$error_output" | grep -qiE "invalid.?api.?key|401|403|unauthorized|UNAUTHENTICATED|authentication|PERMISSION_DENIED|forbidden"; then
        echo "AUTH_ERROR"
    elif echo "$error_output" | grep -qiE "MODEL_CAPACITY_EXHAUSTED|No capacity available"; then
        echo "RATE_LIMIT"
    elif echo "$error_output" | grep -qiE "insufficient.?quota|billing|payment|exceeded.?your.?current.?quota|quota.?exceeded"; then
        ...
        echo "RATE_LIMIT"
    elif echo "$error_output" | grep -qiE "400|invalid.?request|bad.?request|invalid.?argument|INVALID_ARGUMENT"; then
        echo "INVALID_REQUEST"
    elif echo "$error_output" | grep -qiE "500|502|503|504|server.?error|internal.?error|overloaded|INTERNAL|UNAVAILABLE"; then
        echo "SERVER_ERROR"
    elif echo "$error_output" | grep -qiE "ECONNREFUSED|ETIMEDOUT|ENOTFOUND|network|connection|timeout|fetch.?failed|DEADLINE_EXCEEDED"; then
        echo "NETWORK_ERROR"
    elif echo "$error_output" | grep -qiE "context.?length|context.?window|token.?limit|maximum.?context|too.?long|prompt.?too.?large"; then
        echo "CONTEXT_LENGTH"
    else
        echo "UNKNOWN"
    fi
...
run_ai_agent() {
    ...
    local story_id="$2"
    local attempt=0
    local output_log="$RALPH_DIR/gemini_output.log"
    local last_message_file="$RALPH_DIR/gemini_last_message.txt"
    local prompt_file=$(mktemp)
    local current_model="$GEMINI_MODEL"
    local fallback_model="${GEMINI_FALLBACK_MODEL:-gemini-2.5-flash}"
    ...
    while [ $attempt -lt $MAX_RETRIES ]; do
        ...
        if [ $attempt -gt 3 ] && [ "$current_model" != "$fallback_model" ]; then
            log_warning "Switching to fallback model: $fallback_model (primary $GEMINI_MODEL failed $((attempt - 1)) times)"
        ...
        echo -e "${GRAY}---------------------------------------------${NC}"
        echo -e "${CYAN}[$(date '+%H:%M:%S')] Gemini ($current_model) autonomous mode...${NC}"
        echo -e "${GRAY}---------------------------------------------${NC}"

        rm -f "$last_message_file"

        local gemini_stall_timeout=${GEMINI_STALL_TIMEOUT:-300}

        local log_size_before=$(wc -c < "$output_log" 2>/dev/null || echo 0)

        (cd "$WORK_DIR" && cat "$prompt_file" | gemini \
            --yolo \
            --model "$current_model" \
            --no-sandbox \
            -p "" \
        ...
        for _i in $(seq 1 10); do
            gemini_pid=$(pgrep -P $tee_pid 2>/dev/null || pgrep -f "gemini.*--model.*$current_model" 2>/dev/null | head -1)
            [ -n "$gemini_pid" ] && break
            sleep 1
        done
        ...
        while kill -0 $tee_pid 2>/dev/null; do
            ...
            if [ "$current_size" -eq "$last_size" ]; then
                ...
                if [ $stall_seconds -ge $gemini_stall_timeout ]; then
                    log_warning "Gemini stalled for ${gemini_stall_timeout}s - killing"
                    log_activity "Gemini killed (stalled ${gemini_stall_timeout}s, no output)"
                    kill $tee_pid 2>/dev/null
                    pkill -P $tee_pid 2>/dev/null
                    [ -n "$gemini_pid" ] && kill $gemini_pid 2>/dev/null
                    pkill -f "gemini.*--model.*$current_model" 2>/dev/null
                    sleep 2
                    break
                fi
            else
        ...
        echo -e "${GRAY}---------------------------------------------${NC}"
        echo -e "${CYAN}[$(date '+%H:%M:%S')] Gemini finished (exit: $exit_code)${NC}"
        log_activity "Gemini exited with code $exit_code (story: $story_id, model: $current_model)"
        ...
        if [ $attempt_bytes -gt 0 ]; then
            attempt_output=$(tail -c "$attempt_bytes" "$output_log" 2>/dev/null || echo "")
        fi

        local last_output
        last_output=$(echo "$attempt_output" | grep -v "^===" | grep -v "^$" | tail -10)
        if [ -n "$last_output" ]; then
            echo "$last_output" > "$last_message_file"
            local last_msg_preview=$(echo "$last_output" | head -5 | cut -c1-200)
            log_activity "Gemini last message: $last_msg_preview"
        fi

        if [ $exit_code -eq 0 ]; then
            local output_error_type=$(parse_error_type "$attempt_output")
            if [ "$output_error_type" = "UNKNOWN" ]; then
                rm -f "$prompt_file"
                return 0
            fi
            log_warning "Gemini exited 0 but output contains error: $output_error_type"
            exit_code=1
        fi

        local error_type=$(parse_error_type "$attempt_output")
        ...
        case "$error_type" in
            "QUOTA_EXHAUSTED")
                log_warning "Google API capacity/quota issue - retrying with extended backoff"
                local wait_time=$((RATE_LIMIT_WAIT * 2))
                [ $attempt -gt 2 ] && wait_time=$((RATE_LIMIT_WAIT * attempt * 2))
                wait_with_countdown $wait_time "API capacity exhausted ($current_model) - waiting for capacity"
                ;;
            "RATE_LIMIT")
            ...
            "TERMINATED")
                ...
                ;;
            "AUTH_ERROR")
                log_error "Authentication error - run: gemini auth login"
                log_error "Or set GEMINI_API_KEY / GOOGLE_API_KEY environment variable."
                rm -f "$prompt_file"
                return 1
                ;;
            "MODEL_ERROR")
                log_error "Model '$current_model' not found or unavailable. Try gemini-2.5-pro or gemini-2.5-flash."
                rm -f "$prompt_file"
                return 1
                ;;
            "CONTEXT_LENGTH")
            ...
            "SERVER_ERROR")
                ...
                log_warning "Google API server error - retrying with backoff"
                wait_with_countdown $wait_time "Server error (Google)"
                ;;
            *)
                local wait_time=$(calculate_backoff $attempt $INITIAL_BACKOFF)
...
"
    ...
    echo ""
    log_info "Starting Gemini agent ($GEMINI_MODEL)..."

    HEAD_BEFORE_GEMINI=$(git -C "$WORK_DIR" rev-parse HEAD 2>/dev/null || echo "")
    ...
    if ! run_ai_agent "$FULL_PROMPT" "$NEXT_STORY_ID"; then
        ...
        if tail -5 "$ERROR_LOG" | grep -q "AUTH_ERROR\|CONTEXT_LENGTH\|MODEL_ERROR\|QUOTA_EXHAUSTED" 2>/dev/null; then
    ...
    if [ "$CONFIG_LOADED" = "true" ]; then
        HEAD_AFTER_GEMINI=$(git -C "$WORK_DIR" rev-parse HEAD 2>/dev/null || echo "")
        if [ "$HEAD_BEFORE_GEMINI" != "$HEAD_AFTER_GEMINI" ] && [ -n "$HEAD_AFTER_GEMINI" ]; then
            CHANGED_COMMITS=$(($(git -C "$WORK_DIR" rev-list "$HEAD_BEFORE_GEMINI".."$HEAD_AFTER_GEMINI" 2>/dev/null | wc -l | tr -d ' ')))
            log_info "Detected $CHANGED_COMMITS new commit(s) after Gemini iteration"
