...
CLAUDE_MODEL="${CLAUDE_MODEL_ARG:-${CLAUDE_MODEL:-claude-sonnet-4-6}}"
...
CLAUDE_STALL_TIMEOUT=${CLAUDE_STALL_TIMEOUT:-600}
...
PRD_BACKUP="${PRD_BACKUP:-$RALPH_DIR/prd.json.bak}"
PROMPT_FILE="${PROMPT_FILE:-$RALPH_DIR/prompt.md}"
PROGRESS_FILE="${PROGRESS_FILE:-$RALPH_DIR/progress.md}"
...
watch_mode() {
    echo -e "${CYAN}===============================================${NC}"
    echo -e "  Ralph (Claude Code) Watch Mode"
    echo -e "${CYAN}===============================================${NC}"
    echo ""
...
status_mode() {
    echo -e "${CYAN}===============================================${NC}"
    echo -e "  Ralph (Claude Code) Status"
    echo -e "${CYAN}===============================================${NC}"
    echo ""
    ...
    echo -e "   ${GREEN}* $CLAUDE_MODEL (active)${NC}"
    echo -e "   ${GRAY}  $CLAUDE_FALLBACK_MODEL (fallback)${NC}"

    local auth_status
    auth_status=$(claude auth status 2>&1 | python3 -c "import sys,json; d=json.load(sys.stdin); print(f'logged in as {d[\"email\"]} ({d[\"subscriptionType\"]})')" 2>/dev/null || echo "unknown")
    echo ""
    echo -e "${CYAN}Auth:${NC}"
    echo "   $auth_status"
...
echo -e "${CYAN}===============================================${NC}"
echo -e "  Ralph (Claude Code) - Autonomous AI Agent"
echo -e "${CYAN}===============================================${NC}"
echo ""
echo "   Max iterations: $MAX_ITERATIONS"
echo "   Poll interval: ${POLL_INTERVAL}s ($(format_duration $POLL_INTERVAL))"
echo "   Model: $CLAUDE_MODEL"
echo "   Fallback: $CLAUDE_FALLBACK_MODEL"
echo "   Max turns/iter: $MAX_TURNS"
[ -n "$MAX_BUDGET_USD" ] && echo "   Budget cap: \$$MAX_BUDGET_USD/iter"
...
echo -e "${GRAY}   Rate limit wait: ${RATE_LIMIT_WAIT}s | Stall timeout: ${CLAUDE_STALL_TIMEOUT}s${NC}"
echo ""
echo -e "${GRAY}Files:${NC}"
echo -e "${GRAY}   Activity: $ACTIVITY_LOG${NC}"
echo -e "${GRAY}   Errors: $ERROR_LOG${NC}"
echo -e "${GRAY}   Claude output: $RALPH_DIR/claude_output.log${NC}"
echo -e "${GRAY}   Claude last msg: $RALPH_DIR/claude_last_message.txt${NC}"
...
log_activity "=== Ralph (Claude Code) session started ==="
...
if ! command -v claude &> /dev/null; then
    log_error "Claude Code CLI not found. Install with: curl -fsSL https://claude.ai/install.sh | sh"
    exit 1
fi

CLAUDE_VERSION=$(claude --version 2>/dev/null || echo "unknown")
log_success "Using Claude Code CLI ($CLAUDE_VERSION) with model $CLAUDE_MODEL"

CLAUDE_AUTH_JSON=$(claude auth status 2>&1)
if echo "$CLAUDE_AUTH_JSON" | python3 -c "import sys,json; d=json.load(sys.stdin); exit(0 if d.get('loggedIn') else 1)" 2>/dev/null; then
    CLAUDE_USER=$(echo "$CLAUDE_AUTH_JSON" | python3 -c "import sys,json; d=json.load(sys.stdin); print(d.get('email','unknown'))" 2>/dev/null || echo "unknown")
    log_success "Claude Code authenticated as $CLAUDE_USER"
else
    if [ -n "$ANTHROPIC_API_KEY" ]; then
        log_success "Using ANTHROPIC_API_KEY for authentication"
    else
        log_warning "Not authenticated. Run 'claude auth login' or set ANTHROPIC_API_KEY."
        log_info "Attempting to continue - may fail if not authenticated."
    fi
fi
...
parse_error_type() {
    ...
    if echo "$error_output" | grep -qiE "529|overloaded_error|overloaded|This model is currently overloaded|capacity"; then
        echo "OVERLOADED"
    elif echo "$error_output" | grep -qiE "insufficient.?quota|billing|payment|exceeded.?your.?current.?quota|quota.?exceeded|usage.?limit|credit"; then
        echo "QUOTA_EXHAUSTED"
    elif echo "$error_output" | grep -qiE "429|rate.?limit|too.?many.?requests|Rate limit reached|requests.?per.?min|RESOURCE_EXHAUSTED"; then
        echo "RATE_LIMIT"
    elif echo "$error_output" | grep -qiE "Terminated|SIGTERM|SIGKILL|signal|killed"; then
        echo "TERMINATED"
    elif echo "$error_output" | grep -qiE "invalid.?api.?key|401|unauthorized|UNAUTHENTICATED|authentication|not logged in|Invalid API key"; then
        echo "AUTH_ERROR"
    elif echo "$error_output" | grep -qiE "400|invalid.?request|bad.?request|invalid.?argument"; then
        echo "INVALID_REQUEST"
    elif echo "$error_output" | grep -qiE "500|502|503|504|server.?error|internal.?error"; then
        echo "SERVER_ERROR"
    elif echo "$error_output" | grep -qiE "ECONNREFUSED|ETIMEDOUT|ENOTFOUND|network|connection|timeout|fetch.?failed"; then
        echo "NETWORK_ERROR"
    elif echo "$error_output" | grep -qiE "context.?length|context.?window|token.?limit|maximum.?context|too.?long|prompt is too long"; then
        echo "CONTEXT_LENGTH"
    elif echo "$error_output" | grep -qiE "model.?not.?found|does.?not.?exist|invalid.?model|ModelNotFound"; then
        echo "MODEL_ERROR"
    else
        echo "UNKNOWN"
    fi
...
run_ai_agent() {
    ...
    local story_id="$2"
    local attempt=0
    local output_log="$RALPH_DIR/claude_output.log"
    local last_message_file="$RALPH_DIR/claude_last_message.txt"
    ...
    while [ $attempt -lt $MAX_RETRIES ]; do
        attempt=$((attempt + 1))
        [ $attempt -gt 1 ] && log_info "Attempt $attempt of $MAX_RETRIES (model: $CLAUDE_MODEL)..."

        local log_size_before=$(wc -c < "$output_log" 2>/dev/null || echo 0)

        echo "" >> "$output_log"
        echo "=== [$(date '+%H:%M:%S')] Story: $story_id | Model: $CLAUDE_MODEL | Attempt: $attempt ===" >> "$output_log"

        echo -e "${GRAY}---------------------------------------------${NC}"
        echo -e "${CYAN}[$(date '+%H:%M:%S')] Claude Code ($CLAUDE_MODEL) autonomous mode...${NC}"
        echo -e "${GRAY}---------------------------------------------${NC}"
        ...
        (cd "$WORK_DIR" && claude -p "$(cat "$prompt_file")" \
        ...
        while kill -0 $tee_pid 2>/dev/null; do
            ...
            if [ "$current_size" -eq "$last_size" ]; then
                ...
                if [ $stall_seconds -ge $CLAUDE_STALL_TIMEOUT ]; then
                    log_warning "Claude Code stalled for ${CLAUDE_STALL_TIMEOUT}s - killing"
                    log_activity "Claude Code killed (stalled ${CLAUDE_STALL_TIMEOUT}s, no output)"
        ...
        echo -e "${GRAY}---------------------------------------------${NC}"
        echo -e "${CYAN}[$(date '+%H:%M:%S')] Claude Code finished (exit: $exit_code)${NC}"
        log_activity "Claude Code exited with code $exit_code (story: $story_id, model: $CLAUDE_MODEL)"
        ...
        if [ $attempt_bytes -gt 0 ]; then
            ...
            last_msg_preview=$(tail -c "$attempt_bytes" "$output_log" 2>/dev/null \
                | grep -o '"text":"[^"]\{0,300\}' | tail -1 | sed 's/"text":"//' || echo "")
            if [ -n "$last_msg_preview" ]; then
                echo "$last_msg_preview" > "$last_message_file"
                log_activity "Claude last message: ${last_msg_preview:0:200}"
            fi

            local session_id
            session_id=$(tail -c "$attempt_bytes" "$output_log" 2>/dev/null \
                | grep -o '"session_id":"[^"]*"' | head -1 | sed 's/"session_id":"//;s/"//' || echo "")
            if [ -n "$session_id" ]; then
                echo "$session_id" > "$RALPH_DIR/.claude_last_session_id"
            fi
        fi
        ...
        if [ $exit_code -eq 0 ]; then
            ...
            if echo "$error_output" | grep -qiE "rate.?limit|quota.?exceeded|RESOURCE_EXHAUSTED|overloaded|unauthorized|Invalid API key"; then
                log_warning "Claude exited 0 but output contains error: $(parse_error_type "$error_output")"
                exit_code=1
            else
                rm -f "$prompt_file"
                return 0
        ...
        local error_output
        error_output=$(tail -c "${attempt_bytes:-10000}" "$output_log" 2>/dev/null || echo "")
        ...
        log_error "Error in attempt $attempt: $error_type (exit: $exit_code, model: $CLAUDE_MODEL, story: $story_id)"
        echo "[$(date '+%H:%M:%S')] [$story_id] $error_type (exit: $exit_code, model: $CLAUDE_MODEL)" >> "$ERROR_LOG"

        case "$error_type" in
            "OVERLOADED")
                local wait_time=$(calculate_backoff $attempt $INITIAL_BACKOFF)
                [ $wait_time -lt 60 ] && wait_time=60
                log_warning "Anthropic model overloaded (529) - retrying with backoff"
                wait_with_countdown $wait_time "Model overloaded (Anthropic 529)"
                ;;
            "QUOTA_EXHAUSTED")
                log_error "Anthropic quota exhausted - check billing at https://console.anthropic.com"
                rm -f "$prompt_file"
                return 1
                ;;
            "RATE_LIMIT")
                ...
                wait_with_countdown $wait_time "Rate limit reached ($CLAUDE_MODEL)"
                ;;
            "TERMINATED")
                ...
                ;;
            "AUTH_ERROR")
                log_error "Authentication error - run: claude auth login"
                log_error "Or set ANTHROPIC_API_KEY environment variable."
                rm -f "$prompt_file"
                return 1
                ;;
            "MODEL_ERROR")
                log_error "Model '$CLAUDE_MODEL' not found. Available aliases: opus, sonnet, haiku, or full names like claude-sonnet-4-6"
                rm -f "$prompt_file"
                return 1
                ;;
            "CONTEXT_LENGTH")
                log_error "Prompt exceeds token limit for $CLAUDE_MODEL"
                rm -f "$prompt_file"
                return 1
                ;;
            "SERVER_ERROR")
                local wait_time=$(calculate_backoff $attempt $INITIAL_BACKOFF)
                log_warning "Anthropic server error - retrying with backoff"
                wait_with_countdown $wait_time "Server error (Anthropic)"
                ;;
            *)
                local wait_time=$(calculate_backoff $attempt $INITIAL_BACKOFF)
...
"
    ...
    echo ""
    log_info "Starting Claude Code agent ($CLAUDE_MODEL)..."
    ...
    if ! run_ai_agent "$FULL_PROMPT" "$NEXT_STORY_ID"; then
        ...
        if grep -q "AUTH_ERROR\|CONTEXT_LENGTH\|MODEL_ERROR\|QUOTA_EXHAUSTED" "$ERROR_LOG" 2>/dev/null; then
    ...
    if [ "${CONFIG_LOADED:-}" = "true" ]; then
        HEAD_AFTER_CLAUDE=$(git -C "$WORK_DIR" rev-parse HEAD 2>/dev/null || echo "")
        if [ "$HEAD_BEFORE" != "$HEAD_AFTER_CLAUDE" ] && [ -n "$HEAD_AFTER_CLAUDE" ]; then
            CHANGED_COMMITS=$(($(git -C "$WORK_DIR" rev-list "$HEAD_BEFORE".."$HEAD_AFTER_CLAUDE" 2>/dev/null | wc -l | tr -d ' ')))
            log_info "Detected $CHANGED_COMMITS new commit(s) after Claude iteration"
    ...
    if [ -f "$ERROR_LOG" ]; then
        RECENT_RATE_LIMITS=$(tail -5 "$ERROR_LOG" 2>/dev/null | grep -c "RATE_LIMIT\|OVERLOADED" || echo "0")
        [[ "$RECENT_RATE_LIMITS" =~ ^[0-9]+$ ]] || RECENT_RATE_LIMITS=0
        if [ "$RECENT_RATE_LIMITS" -ge 2 ]; then
            log_warning "Multiple rate limits/overloads - extended pause"
...
if [ -f "$ERROR_LOG" ]; then
    ...
    if [ "$ERROR_COUNT" -gt 0 ]; then
        ...
        grep -oE "RATE_LIMIT|OVERLOADED|INVALID_REQUEST|SERVER_ERROR|NETWORK_ERROR|QUOTA_EXHAUSTED|AUTH_ERROR|MODEL_ERROR|CONTEXT_LENGTH|UNKNOWN" "$ERROR_LOG" 2>/dev/null | sort | uniq -c | while read count type; do
            echo "    - $type: $count"
        done
    fi
fi
