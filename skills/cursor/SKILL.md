...
**Failure Memory** (Rust only) — Tracks every failed attempt per story. When consecutive attempts use the same approach, triggers "diversity enforcement": the prompt tells the agent to try a different strategy and lists banned approaches.

**Gutter Detection** — When a story fails more than `GUTTER_THRESHOLD` times, it's auto-blocked and a guardrail is added. Prevents infinite loops on impossible tasks.

**Stall Detection** — Monitors agent stdout. If no output for `stall_timeout` seconds, the process is killed (SIGINT -> SIGTERM -> SIGKILL) and retried.

**Loop Detection** — Analyzes output for repeating patterns (windows of 1-3 lines repeated 3+ times). Logs a warning when detected.

**Guardrails** — Lessons learned from failures. Read by the agent at the start of every iteration. Added automatically on repeated failures, or manually by the user.
